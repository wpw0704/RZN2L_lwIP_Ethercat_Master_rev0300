...
#else
/* IAR / non-GCC: use inline function pattern instead of statement expression.
 * Callers must use: get_unaligned16(&val), get_unaligned32(&val) etc. */
static inline uint16 get_unaligned16(const void *ptr) { uint16 v; memcpy(&v, ptr, 2); return v; }
static inline uint32 get_unaligned32(const void *ptr) { uint32 v; memcpy(&v, ptr, 4); return v; }
static inline uint64 get_unaligned64(const void *ptr) { uint64 v; memcpy(&v, ptr, 8); return v; }
/* Provide a generic macro that works for known sizes via _Generic (C11) */
#define get_unaligned(ptr) \
  _Generic((*(ptr)),       \
    uint16: get_unaligned16(ptr), \
    int16:  get_unaligned16(ptr), \
    uint32: get_unaligned32(ptr), \
    int32:  get_unaligned32(ptr), \
    uint64: get_unaligned64(ptr), \
    int64:  get_unaligned64(ptr)  \
  )
#endif
