...
static int ethercat_extract_wkc(uint8_t const *p_ecat, int payload_length);
...
static int ethercat_extract_wkc(uint8_t const *p_ecat, int payload_length) {
    uint16_t ethercat_length;

    if ((p_ecat == NULL) || (payload_length < 4)) {
        return EC_NOFRAME;
    }

    /*
     * EtherCAT帧长度位于EtherCAT头部前两个字节的低12位。
     * SOEM接收缓冲从EtherCAT头开始，不包含14字节以太网头。
     */
    ethercat_length =
            (uint16_t) p_ecat[0] |
            ((uint16_t) (p_ecat[1] & 0x0FU) << 8);

    if (((int) ethercat_length + 1) >= payload_length) {
        return EC_NOFRAME;
    }

    return (int) p_ecat[ethercat_length] |
           ((int) p_ecat[ethercat_length + 1U] << 8);
}
    ...
    do {
        ...
        if ((port->rxbufstat[idx] == EC_BUF_RCVD) ||
            ...
            int expected_length =
                    ...
                    (int) ETHERCAT_FRAME_HEADER;

            wkc = ethercat_extract_wkc(
                port->rxbuf[idx],
                expected_length);
