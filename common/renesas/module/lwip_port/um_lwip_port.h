...
{
    SemaphoreHandle_t                 p_mutex_handle;    /** Pointer to mutex for controlling exclusion of
                                                         *   callback resource. */
...
{
    struct st_lwip_port_netif_ctrl * p_next;                /** Pointer to next lwIP network interface to
                                                            *   be managed by network interface list.*/
    ...
    ether_netif_frame_t *            p_ether_netif_frame;    /** Ethernet frame packet buffer for getting
                                                             *it from queue. */
...
{
    SemaphoreHandle_t        p_mutex_handle;    /** Pointer to mutex for controlling exclusion of the
                                                *   list of network controllers. */
...
typedef struct st_lwip_port_launcher_ctrl
{
    ...
    QueueHandle_t    p_queue_handle;        /** Pointer to queue handle for notifying that the Ethernet
                                            *   network interface is up. */
    void *           p_netif_list_ctrl;    ///< Pointer to network interface list control to be switched to up/down.
} lwip_port_launcher_ctrl_t;

/**
 * Struct for the task control for receiving Ethernet frame from Ethernet network interface and passing
   it into lwIP network interface.
 */
typedef struct st_lwip_port_receiver_ctrl
{
    TaskHandle_t          p_task_handle;            /** Pointer to task handle for passing
                                                    *     Ethernet frame from Ethernet network interface
                                                    *     into lwIP network interface. */
    TaskHandle_t          p_parent_task_handle;
    QueueHandle_t         p_queue_handle;            /** Pointer to queue handle for passing Ethernet frame
                                                     *   from Ethernet network interface into the task. */
    ether_netif_frame_t * p_ether_netif_frame;       ///< Ethernet frame packet buffer for getting it from queue.
    void *                p_netif_list_ctrl;         /**  Pointer to network interface list controller to
                                                     *    be passed received Ethernet frames. */
...
{
    ether_netif_instance_t           *p_ether_netif_instance;    ///< Pointer to Ethernet network interface instance.
    ether_netif_callback_args_t       callback_memory;          /** Callback memory returned as argument when the
                                                                *   callback Ethernet network interface is called. */
    ether_netif_callback_link_node_t  callback_node;            /** Callback node for utilizing callback of Ethernet
                                                                *   network interface. */
    void *                            p_launcher_ctrl;          /** Pointer to launcher task control to be notified
                                                                *  that Ethernet network interface is up. */
    void *                            p_receiver_ctrl;          /** Pointer to receiver task control to be passed
                                                                *   Ethernet frame from Ethernet network interface. */
...
typedef struct st_lwip_port_common_cfg
{
    ...
    const struct st_lwip_port_common_ctrl * p_common_ctrl;            /** Pointer to common controller over multiple
                                                                      *   instance. */
